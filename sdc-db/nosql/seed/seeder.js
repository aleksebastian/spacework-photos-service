...
const seed = async () => {
  ...
  let optimizedPhotoUrls = photoUrls.map(
    (url) =>
      (url = `${url.slice(
        0,
        url.indexOf("upload/") + strLength
      )}f_auto,q_auto,w_1600/${url.slice(url.indexOf("upload/") + strLength)}`)
  );
  ...
  let PrimaryRecordBatchInserts = Math.floor(
    primaryRecordCount / batchInsertCount
  );
  ...
  for (let i = 0; i < PrimaryRecordBatchInserts; i++) {
    ...
    } else if (i / PrimaryRecordBatchInserts === 0.5) {
