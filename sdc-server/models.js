...
const getPhotoById = async (workspaceId, id) => {
  return new Promise(async (resolve, reject) => {
    try {
      ...
      let photoInfo = workspacePhotosDoc.photos.find(
        (photo) => photo.id === id
      );
      resolve(photoInfo);
    } catch (e) {
...
const updatePhoto = async (workspaceId, id, url, description) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      let updatePhotos = await db.insert({
        _id: _id,
        _rev: _rev,
        workspace_id: workspaceId,
...
const deletePhotosByWorkspaceId = async (workspaceId) => {
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      let removePhotos = await db.insert({
        _id: _id,
        _rev: _rev,
        workspace_id: workspaceId,
        photos: emptyPhotos,
